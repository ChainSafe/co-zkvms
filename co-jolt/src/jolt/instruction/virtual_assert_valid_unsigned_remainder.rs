...
use crate::utils::instruction_utils::{rep3_chunk_and_concatenate_operands, transpose};
    ...
    for AssertValidUnsignedRemainderInstruction<WORD_SIZE, F>
{
    ...
    fn combine_lookups_rep3_batched<N: Rep3Network>(
        ...
        #[cfg(feature = "public-eq")]
        let eq = rep3::arithmetic::open_vec(&vals_by_subtable_by_term[1].concat(), io_ctx)?; // todo

        #[cfg(not(feature = "public-eq"))]
        let divisor_is_zero = rep3::arithmetic::product_many(
            &transpose(std::mem::take(&mut vals_by_subtable_by_term[2])),
            io_ctx,
        )?;
        #[cfg(feature = "public-eq")]
        let divisor_is_zero = {
            let divisor_is_zero_vals =
        ...
        let ltu_sum_eq_prod = ltu
            ...
            .map(|(ltu, eq_prod)| ltu[C - 1] * eq_prod)
            .collect::<Vec<_>>();
        #[cfg(feature = "public-eq")]
        let ltu_sum_eq_prod = ltu
            .iter()
        ...
        let res = rep3::arithmetic::reshare_additive_many(
            &sums
                .iter()
                .zip(ltu_sum_eq_prod.iter())
                .map(|(sum, ltu_sum_eq_prod)| *sum + *ltu_sum_eq_prod)
                .collect::<Vec<_>>(),
            io_ctx,
        )?
        .into_iter()
        .zip(divisor_is_zero.into_iter())
