...
impl<const C: usize, F: JoltField> R1CSConstraints<C, F> for JoltRV32IMConstraints {
    ...
    fn uniform_constraints(cs: &mut R1CSBuilder<C, F, Self::Inputs>, memory_start: u64) {
        ...
        let flags = CircuitFlags::iter()
            ...
            .chain(
                RV32I::iter().map(|flag| JoltR1CSInputs::InstructionFlags(flag).into()),
            )
            .collect();
        ...
        let y_chunks: Vec<Variable> = (0..C)
            ...
            .collect();
        let x_concat =
            R1CSBuilder::<C, F, JoltR1CSInputs>::pack_be(x_chunks.clone(), OPERAND_SIZE);
        let y_concat =
            R1CSBuilder::<C, F, JoltR1CSInputs>::pack_be(y_chunks.clone(), OPERAND_SIZE);
        cs.constrain_eq_conditional(
