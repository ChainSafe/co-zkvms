...
use super::sumcheck::CubicSumcheckType;
...
pub struct GrandProductCircuit<F: JoltField> {
    pub left_vec: Vec<DensePolynomial<F>>,
    pub right_vec: Vec<DensePolynomial<F>>,
}
...
pub struct LayerProofBatched<F: JoltField, ProofTranscript: Transcript> {
...
#[allow(dead_code)]
impl<F: JoltField, ProofTranscript: Transcript> LayerProofBatched<F, ProofTranscript> {
    pub fn verify(
        &self,
        claim: F,
        num_rounds: usize,
...
pub struct BatchedGrandProductCircuit<F: JoltField> {
    ...
    pub fingerprint_polys: Option<Vec<DensePolynomial<F>>>,
}
...
{
    pub proof: Vec<LayerProofBatched<PCS::Field, ProofTranscript>>,
}
        ...
        PCS: CommitmentScheme<ProofTranscript, Field = F>,
{
    ...
    pub fn prove(
        ...
        for layer_id in (0..batch.num_layers()).rev() {
            ...
            let coeff_vec: Vec<F> = transcript.challenge_vector(claims_to_verify.len());
            ...
            let params = batch.sumcheck_layer_params(layer_id, eq.clone());
    ...
    pub fn verify(
        ...
        for (num_rounds, i) in (0..num_layers).enumerate() {
            ...
            let coeff_vec = transcript.challenge_vector::<F>(claims_to_verify.len());
...
pub fn batch_sumcheck_layer_params<F: JoltField>(
    batch: &mut BatchedGrandProductCircuit<F>,
    layer_id: usize,
    eq: DensePolynomial<F>,
) -> CubicSumcheckParams<F> {
    if batch.flags_present && layer_id == 0 {
        let flags = batch.flags.as_ref().unwrap();
        debug_assert_eq!(flags[0].len(), eq.len());

        let num_rounds = eq.get_num_vars();

        // Each of these is needed exactly once, transfer ownership rather than clone.
        let fingerprint_polys = batch.fingerprint_polys.take().unwrap();
        let flags = batch.flags.take().unwrap();
        CubicSumcheckParams::new_flags(fingerprint_polys, flags, eq, num_rounds)
    } else {
        // If flags is present layer_id 1 corresponds to circuits.left_vec/right_vec[0]
        let layer_id = if batch.flags_present {
            layer_id - 1
        } else {
            layer_id
        };

        let num_rounds = batch.circuits[0].left_vec[layer_id].get_num_vars();

        let (lefts, rights): (Vec<DensePolynomial<F>>, Vec<DensePolynomial<F>>) = batch
            .circuits
            .iter_mut()
            .map(|circuit| circuit.take_layer(layer_id))
            .unzip();
        if batch.flags_present {
            CubicSumcheckParams::new_prod_ones(lefts, rights, eq, num_rounds)
        } else {
            CubicSumcheckParams::new_prod(lefts, rights, eq, num_rounds)
        }
