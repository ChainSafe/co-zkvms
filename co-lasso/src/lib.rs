...
use co_spartan::mpc::rep3::{Rep3BigUintShare, Rep3Poly};
...
pub fn polynomialize<F: JoltField>(
    ...
    let subtable_lookup_indices: Vec<Vec<Rep3BigUintShare<F>>> =
        subtable_lookup_indices(preprocessing, inputs, &lookups, M, C);
...
fn subtable_lookup_indices<F: JoltField>(
    ...
    let indices: Vec<_> = (0..num_rows)
        ...
        .map(|(i, lookup_id)| {
            ...
            //     );
            // }
            let mut chunked_index = iter::repeat(Rep3BigUintShare::zero()).take(num_chunks).collect_vec();
